...
$(document).ready(function () {
    ...
    // 커서를 연한 회색으로 바꾸고 싶은 특정 div의 클래스명을 지정합니다.
    var grayZone = '.change-gray-zone';

    $(document).on('mouseenter', grayZone, function () {
        // 해당 구역에 진입하면 회색 스타일 클래스를 부여합니다.
        cursorDot.addClass('is-gray');
        cursorFollower.addClass('is-gray');
    });

    $(document).on('mouseleave', grayZone, function () {
        // 구역을 벗어나면 회색 클래스를 제거하여 원래대로 복원합니다.
        cursorDot.removeClass('is-gray');
        cursorFollower.removeClass('is-gray');
    });
});
